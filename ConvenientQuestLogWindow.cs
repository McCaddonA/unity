    ...
    public class ConvenientQuestLogWindow : DaggerfallQuestJournalWindow
    {
        ...
        private void SetTextActiveQuests()
        {
            ...
            foreach (var parent in parents)
            {
                ...
                var message = questMessages.Where(x => x.ParentQuest == parent && x.ID == lastLogEntry.messageID).Single();
            ...
            for (int i = currentMessageIndex; i < groupedQuestMessages.Count; i++)
            {
                ...
                questTitle = FormatMainQuestTitle(questTitle);
                ...
                if (!string.IsNullOrWhiteSpace(lastPlaceMentioned?.SiteDetails.locationName))
                {
                    ...
                    tokens.Add(new TextFile.Token(TextFile.Formatting.Text, locationAndTravelTime));
                }
        ...
        private string FormatMainQuestTitle(string questTitle)
        {
